...
但还要注意一件事：在绝大多数情况下，一个好的模型只是近似，可以接近但不会等于事实。模型之所以有用，是因为在我们关心的范围内，它已经足够好地描述了 $x$ 和 $y$ 之间的关系。
...
这两个函数当然完全不同。$e^x$ 是指数增长，$\ln x$ 是缓慢增长。但在这里，如果们只关心 $x$ 接近 1 的一小段范围，而且我们有这么一段数据，在不知道真实关系的情况下，为了描述这段局部关系，我们可以先选用同一个二次模型形式：
