...
这也是为什么很多模型问题，根本原因不在 `M`，而在 `X`。如果 feature 缺少关键信息，模型无法凭空恢复。预测房价时，输入只有面积而没有城市和地段，再强的模型也只能平均猜测；判断文本情感时，如果表征只保留词频、不保留顺序，就很难区分 “good” 和 “not good”。

Feature 还会改变问题的几何形状。原始空间中纠缠在一起的样本，经过合适表征后可能变得容易分开。核方法、embedding、深度网络的中间层，本质上都在改变数据所在空间，让后续 `M` 更容易把 `X` 映射到 `Y`。

自动表征学习不等于人类不用思考。我们仍然要决定输入包含什么、tokenizer 如何切分、图像如何预处理、时间如何编码、ID 如何处理、缺失值如何填补，以及哪些 feature 可能泄漏标签。工业系统中，线上效果突然变差，常常不是模型结构退化，而是某个 feature 延迟、缺失、含义变化，或训练/推理不一致。
...
- <em>传统方式：人设计 feature -&gt; 模型学习预测器</em>
- <em>深度学习：模型同时学习 feature 和预测器</em>
...
- <em>离散对象 -&gt; embedding</em>
- <em>连续对象 -&gt; normalization / scaling</em>
- <em>序列对象 -&gt; positional encoding + sequence model</em>
- <em>图像对象 -&gt; patch / convolution / vision transformer</em>
...
One-hot 简单、明确，但有两个缺点：维度大，且不表达相似性。`cat` 和 `dog` 都是动物，但在 one-hot 空间中，它们并不天然更近。

### 4.1 稀疏特征的工业价值

虽然 one-hot 看起来笨，但稀疏特征在很多工业系统中非常有用。广告、推荐、搜索系统里，经常有大量离散 ID 和类别特征。

稀疏特征的优点是精确记忆。例如某个广告 ID、某个用户群、某个查询词可能有稳定统计信号。缺点是冷启动和泛化困难。新 ID 没有历史数据，模型就很难判断。

## 第5节：Embedding，把符号放进几何空间
...
这就是许多 RAG 系统的基础。语言模型负责生成，向量检索负责把外部知识带入上下文。
...
第四，泄漏。feature 中包含了不该在预测时可见的信息，导致离线效果虚高。
...
- <em>timestamp -&gt; hour_of_day, day_of_week, is_holiday, recency</em>
...
## 第11节：图像表征，从像素到 patch

图像最原始的表征是像素。一个彩色图像可以表示成：

- <em>[height, width, channel]</em>
...
图像表征也会遇到数据增强。随机裁剪、翻转、颜色扰动，不只是让数据变多，而是在告诉模型：这些变化不应该改变标签。数据增强本质上是在向模型注入不变性。
...
时间表征的最大风险是未来泄漏。例如预测用户明天是否购买时，如果 feature 包含明天之后的行为，离线效果会非常好，上线必然失败。
...
- <em>商品 ID embedding + 商品标题 embedding + 类目 + 价格 + 图像特征</em>
...
- <em>city = Beijing</em>
- <em>device = mobile</em>
- <em>cross = Beijing_mobile</em>

深度学习中，模型可以通过 embedding、MLP、attention 自动学习交叉。但自动学习需要数据。如果某些组合很稀疏，手工交叉或统计特征仍然有价值。

特征交叉的风险是维度爆炸。两个各有 10000 种取值的类别特征，交叉后理论上有 1 亿种组合。大部分组合很少出现，容易过拟合。
...
表征公平性没有简单答案，但有一个基本原则：不要让平均指标掩盖分群伤害。
...
每一步都可能出错。清洗可能删掉重要样本，关联可能产生重复或错配，转换可能引入泄漏，验证可能漏掉分布变化。

训练和推理最好共享同一套转换逻辑。如果训练用 Python 离线脚本，推理用 C++ 在线实现，两者就可能产生细微差异。这种差异很难从模型代码中发现。

因此，构造 `X` 是一等工程问题。它不是模型之前的小准备，而是学习系统的一部分。

### 本章小结

- `X` 不是现实本身，而是现实的可计算表征。
- Feature engineering 是把人类理解写进输入。
- Embedding 是把离散对象放入可学习几何空间。
- 表征学习是深度学习区别于传统方法的关键。

### 练习题

1. 对“预测一篇文章是否会被用户点击”这个任务，列出 10 个可能 feature，并说明哪些可能有泄漏风险。
2. 为什么 one-hot 不能表达“cat”和“dog”比“cat”和“airplane”更相似？
3. Embedding table 是查表，为什么它仍然能泛化？
4. 对一个 RAG 系统，query embedding 和 document embedding 分别扮演什么角色？
5. 举一个表征分布漂移的例子。
