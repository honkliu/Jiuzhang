...
Ask a question against readable files in `Raw/`:

```powershell
node src/cli.js knowledge ask "what is the readme about"
```

This command first retrieves candidate snippets from local files in `Raw/`. If `Config/shang.json` has `llm.baseUrl`, `llm.apiKey`, and `llm.model`, Shang loads the `knowledge-ask` skill from `Agents/skills/knowledge-ask/` and sends that skill prompt plus retrieved snippets to the configured OpenAI-compatible model. If LLM config is missing, it falls back to local snippets.

Use `--limit` to change how many matching documents are included:

```powershell
node src/cli.js knowledge ask "what is the readme about" --limit 5
```

Use a specific skill prompt:

```powershell
node src/cli.js knowledge ask "what is the readme about" --skill knowledge-ask
```

Force the non-LLM fallback:

```powershell
node src/cli.js knowledge ask "what is the readme about" --llm false
```
